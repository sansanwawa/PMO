...
public class ProjectController {
    ...
    public void json(HttpServletRequest request, HttpServletResponse response, Project project) throws Exception {
    HttpServletResponseWrapper responseWrapper = new HttpServletResponseWrapper(response);
    ...
    //limit
    projectDAO.setMaxResults(Integer.parseInt(request.getParameter("limit")));
    //order by
    projectDAO.orderBy(request.getParameter("sort"), request.getParameter("dir"));
    List listProject = (List) projectDAO.list(Integer.parseInt(request.getParameter("start"))).get(0);
    ...
    public void add(HttpServletRequest request, HttpServletResponse response, Project project) throws Exception {
    ...
    //return new ModelAndView("project/projectForm", modelMap);
    }
    ...
    Project project = (Project) this.newCommandObject(Project.class);
    ...
    project.setProjectStartDate(dateStart);
    project.setProjectEndDate(dateEnd);


    this.bind(request, project);
    ...
    public ModelAndView list(HttpServletRequest request, HttpServletResponse response, Project project) throws Exception {
    ...
    if (sort.equals("desc")) {
    projectDAO.orderByAsc(field);
    modelMap.addAttribute("sort", "asc");
    } else {
    projectDAO.orderByDesc(field);
    modelMap.addAttribute("sort", "desc");
    }
    }

    List TotalData = (List) projectDAO.list(offset).get(1);
    modelMap.addAttribute("projectList", projectDAO.list(offset).get(0));
    modelMap.addAttribute("totalData", TotalData.get(0));
    modelMap.addAttribute("maxPage", projectDAO.list(offset).get(2));
    return new ModelAndView("project/projectList", modelMap);
    }

    public void delete(HttpServletRequest request, HttpServletResponse response, Project project) throws Exception {
    String project_id = request.getParameter("id");
    Writer out = response.getWriter();
    ...
    p.setId(Long.parseLong(project_ids[i]));
    this.projectDAO.delete(p);
    logger.info("delete project_id = " + project_ids[i]);
    }
